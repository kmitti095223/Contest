...
void sub(char* T, int Q, int XX);
void sub_1(char* T, int Q);
void sub_2(char* T, int Q);
...
inline int countChars2(const char* str) {
    int count = 0;
    while (count < 11 && str[count] != '\0') {
        count++;
    }
    return  count;
}
...
// 文字列のインデックスを計算する関数
inline int calculateIndex2(const char* str, char base,int max=10) {
    const int BASE = 4; // 基数
    int index = 0;
    int length = countChars2(str);

    for (int i = 0; i < length && i < max; i++) {
        index = index * BASE + (str[i]== base ? 1 : 2);
    }
...
void sub_1(char *T, int Q)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        index1 = charToidx(T[i]);

        if (T[i + 1]) {
            index2 = index1 * 62 + charToidx(T[i + 1]);

            if (T[i + 2]) {
                index3 = index2 * 62 + charToidx(T[i + 2]);

                if (T[i + 3]) {
                    index4 = index3 * 62 + charToidx(T[i + 3]);
                }
            }
        }
...
void sub(char* T, int Q,int XX)
{
    ...
    for (int i = 0; i < Q; i++) {
        ...
        int index = calculateIndex2(P[i], P[0][0]);

        int v = countChars2(P[i]);
        if (v > XX)
            prefixCheck[index] = 2;
        else
            prefixCheck[index] = 1;
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < XX; j++) {
            ...
            index[j] = calculateIndex2(&T[i], P[0][0], j + 1);
...
//            }
    ...
    for (int i = 0; i < Q; i++) {
        ...
        int index1 = calculateIndex2(P[i], P[0][0]);

        if (prefixCheck2[index1])
        {
            int len = countChars2(P[i]);
            if (len == (XX+1)) {
                // bsearchを使用して二分探索
                char** result = (char**)bsearch(P[i], suffixArray, suffixArraySize, sizeof(char*), prefixCompare);
                if (result != NULL) {
...
//            }
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if (strcmp(P[i], "bbaaabbabbbbbaaa") == 0)
            int x = 1;
