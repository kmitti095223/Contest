...
int main() {
    ...
    WK = new   short[PREFIX_INDEX_SIZE];
    P_Exists = new   char[PREFIX_INDEX_SIZE];
    prefixFlagTable = new   char[PREFIX_INDEX_SIZE];
    memset(WK, -1, sizeof(short) * PREFIX_INDEX_SIZE);
    memset(P_Exists, 0, sizeof(char) * PREFIX_INDEX_SIZE);
    memset(prefixFlagTable, 0, sizeof(char) * PREFIX_INDEX_SIZE);
    ...
    int nStart = len_min - 1;
    if (nStart >= maxPrefixLength)
    {
        nStart = maxPrefixLength - 1;
    }
    ...
    for (int i = 0; i < textLength - len_min + 1; i++) {
        ...
        for (int j = nStart; j < maxPrefixLength; j++) { // このforループが遅い
            if (T[i + j] == 0)
                break;

            // maxPrefixLength2 = j+1;
            if (j == nStart) {
                for (int k = 0; k<nStart; k++)
                {
                    tempIndex = tempIndex * characterBase + characterMap[T[i + k]];
                }
            }

            // tempIndex = computePrefixIndex(&T[i]);
            tempIndex = tempIndex * characterBase + characterMap[T[i + j]];
            if (P_Exists[tempIndex]) {

                //if (tempIndex == 52)
                //    i = i;

                prefixFlagTable[tempIndex] = 1;


                if (P_Exists[tempIndex] == 2)
                    flag = 1;
            }
        ...
        if (flag) {
            IndexEx = prefixIndices[maxPrefixLength - 1];
            if (WK[IndexEx] >= 0) {
                // printf(P[WK[prefixIndices[maxPrefixLength - 1]]]);

                if (P2[WK[IndexEx]] != 1) {

                    if (strncmp(P[WK[IndexEx]], currentSuffix, P_Len[WK[IndexEx]]) == 0)
                        P2[WK[IndexEx]] = 1;
                    else {
                        P2[WK[IndexEx]] = -1;
                    }
