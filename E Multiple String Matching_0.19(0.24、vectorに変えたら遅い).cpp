...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (prefixCheck[index1] || prefixCheck[index2] || prefixCheck[index3] || prefixCheck[index4])
        {
            ...
            if (suffixArraySize[idx] == 0 || strncmp(suffixArray[idx][suffixArraySize[idx] - 1], currentSuffix, 1000) != 0) {
                ...
                vec[idx].push_back(currentSuffix);
            }
        }
    }
    ...
    for (int i = 0; i < 62; i++) {
     //    qsort(suffixArray[i], suffixArraySize[i], sizeof(char*), compare);
        ...
        std::sort(vec[i].begin(), vec[i].end(), [](const char* a, const char* b) {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        {
            g_len = countChars(P[i]);
            if (g_len == 5) {

                g_len = strlen(P[i]);
                 if (std::binary_search(vec[idx].begin(), vec[idx].end(), P[i], cmp))
                 {
