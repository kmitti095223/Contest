...
void limit_consecutive_chars(char* T, int& TLen) {
    if (TLen == 0) return;

    int write_index = 1;  // 書き込み位置
    int count = 1;        // 連続カウント
...
int main() {
    ...
    *ptr++ = '\0';  // 改行をNULLに置換し次に進む
    ...
    int mask = (int)pow(baseSize, nPrefixLength);
    int mask2 = (int)pow(baseSize, nPrefixLength-1);

    int prev_index = calculateIndex(T, nPrefixLength, charPresence, baseSize);
    prev_index = prev_index * baseSize;
    prev_index = prev_index % mask;

    limit_consecutive_chars(T, TLen);
    ...
    for (int i = 0; i <= TLen - minPLength; ++i) {  // Tの長さから最短Pの長さを引いた範囲でループ
        ...
        if (minPLength >= nPrefixLength) {
            index = prev_index / baseSize;
            index += (charPresence[T[i + nPrefixLength - 1]] * mask2);
        ...
        for (int j = 0; j < nPrefixLength; ++j) {
            ...
            if (minPLength >= nPrefixLength && j < nPrefixLength - 1)
                continue;
