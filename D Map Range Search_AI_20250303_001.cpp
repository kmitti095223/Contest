...
constexpr int ALPHABET_SIZE = 27;  // アルファベット + 0 の格納用

// キーの3文字を基にインデックスを計算 (1～3文字対応)
constexpr int calculate_index(const char* key, size_t len) {
    int index = 0;
    if (len >= 1) index += ((key[0] - 'a' + 1) * ALPHABET_SIZE * ALPHABET_SIZE);  // 'a' -> 1, ..., 'z' -> 26
    if (len >= 2) index += ((key[1] - 'a' + 1) * ALPHABET_SIZE);                 // 'a' -> 1, ..., 'z' -> 26
    if (len >= 3) index += ((key[2] - 'a' + 1));                                // 'a' -> 1, ..., 'z' -> 26
    return index;
}
...
// 定数
FlatMap flat_maps[ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE];  // 27^3 = 19,683個のFlatMap
